...
class CommentApiTests(TestCase):
    ...
    def test_list(self):
        #tweet_id must be included in the request
        response = self.anonymous_client.get(COMMENT_URL)
        self.assertEqual(response.status_code, 400)

        # list method is accessed when tweet_id is available
        # no comments
        response = self.anonymous_client.get(COMMENT_URL, {
            'tweet_id': self.tweet.id,
        })
        ...
        # request includes tweet_id and user_id, only tweet_id gets used
        response = self.anonymous_client.get(COMMENT_URL, {
            'tweet_id': self.tweet.id,
